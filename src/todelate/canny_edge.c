...
int read_pgm_image(char *infilename, char **image, int *rows,
    int *cols);
int write_pgm_image(char *outfilename, char *image, int rows,
    int cols, char *comment, int maxval);

void canny(char *image, int rows, int cols, float sigma,
         float tlow, float thigh, char **edge);
void gaussian_smooth(char *image, int rows, int cols, float sigma,
...
void magnitude_x_y(short int *delta_x, short int *delta_y, int rows, int cols,
        short int **magnitude);
void apply_hysteresis(short int *mag, char *nms, int rows, int cols,
        float tlow, float thigh, char *edge);
...
* DATE: 2/15/96
*******************************************************************************/
void follow_edges(char *edgemapptr, short *edgemagptr, short lowval,
   int cols)
{
   short *tempmagptr;
   char *tempmapptr;
    ...
    char *result) 
{
   ...
   ****************************************************************************/
    for(count=0,resultrowptr=result,resultptr=result+ncols*(nrows-1); 
        ...
        *resultrowptr = *resultptr = (char) 0;
    }

    for(count=0,resultptr=result,resultrowptr=result+ncols-1;
        count<nrows; count++,resultptr+=ncols,resultrowptr+=ncols){
        *resultptr = *resultrowptr = (char) 0;
    }
    ...
    int *cols)
{
   ...
   ***************************************************************************/
   if(((*image) = (char *) malloc((*rows)*(*cols))) == NULL){
...
* image. Upon failure, this function returns 0, upon sucess it returns 1.
******************************************************************************/
int read_ppm_image(char *infilename, char **image_red, 
    char **image_grn, char **image_blu, int *rows,
    int *cols)
{
   FILE *fp;
   char buf[71];
   ...
   * Allocate memory to store the image then read the image from the file.
   ***************************************************************************/
   if(((*image_red) = (char *) malloc((*rows)*(*cols))) == NULL){
      fprintf(stderr, "Memory allocation failure in read_ppm_image().\n");
      if(fp != stdin) fclose(fp);
      return(0);
   }
   if(((*image_grn) = (char *) malloc((*rows)*(*cols))) == NULL){
      fprintf(stderr, "Memory allocation failure in read_ppm_image().\n");
      if(fp != stdin) fclose(fp);
      return(0);
   }
   if(((*image_blu) = (char *) malloc((*rows)*(*cols))) == NULL){
      fprintf(stderr, "Memory allocation failure in read_ppm_image().\n");
      if(fp != stdin) fclose(fp);
      return(0);
   }
   ...
   size = (*rows)*(*cols);
   for(p=0;p<size;p++){
      (*image_red)[p] = (char)fgetc(fp);
      (*image_grn)[p] = (char)fgetc(fp);
      (*image_blu)[p] = (char)fgetc(fp);
   }
...
* outfilename = NULL. A comment can be written to the header if coment != NULL.
******************************************************************************/
int write_ppm_image(char *outfilename, char *image_red,
    char *image_grn, char *image_blu, int rows,
    int cols, char *comment, int maxval)
{
   FILE *fp;
         ...
         float tlow, float thigh, char **edge)
{
   ...
   char *nms;        /* Points that are local maximal magnitude. */
   ...
   ****************************************************************************/
   ...
   if((nms = (char *) calloc(rows*cols,sizeof(char)))==NULL){
   ...
   ****************************************************************************/
   ...
   if((*edge=(char *)calloc(rows*cols,sizeof(char))) ==NULL){
